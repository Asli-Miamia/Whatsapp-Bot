...
const cuaca = async (provinsi, kota) => new Promise((resolve, reject) => {
    axios.get(`https://cuaca-gempa-rest-api.vercel.app/weather/${provinsi}/sukabumi${kota}`)
    .then((res) => {
		if (res.success == null) resolve('Maaf daerah kamu tidak tersedia')
        const text = `
Cuaca di: provinsi: ${res.data.domain}, kota: ${res.data.description}
Garis Lintang: ${res.data.latitude}
Garis Lujur: ${res.data.longitude}
Kordinat: ${res.data.coordinate}
Cuaca: ${res.data.level}
