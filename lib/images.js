 ...
 */
 ...
 const fdci = async (image) => new Promise((resolve, reject) => {
     fetchJson('http://api.fdci.se/rep.php?gambar=' + image)
        .then((result) => {
            const andimage = Math.floor(Math.random() * 41)
            resolve(result[andimage])
        })
        .catch((err) => {
            reject(err)
        })
