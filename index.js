...
const start = async (client = new Client()) => {
    console.log(color(figlet.textSync('AMADEUS BOT', { font: 'Ghost', horizontalLayout: 'default' })))
        console.log('[SERVER] AMADEUS Started!')
        ...
        client.onAddedToGroup(((chat) => {
            ...
            if (totalMem < 257) { 
            	client.sendText(chat.id, `Halo warga ${name} mohon maaf, jika mau menginvite bot, silahkan membeli akses premium bot melalui wa.me//628119001453`).then(() => client.leaveGroup(chat.id)).then(() => client.deleteChat(chat.id))
            } else {
    ...
    await client.sendText(chat.id, `Sorry, bot tidak bisa join di grub untuk menginvite bot silahkan hubungi owner bot https://wa.me/628119001453 max group: ${groupLimit}`).then(() => {
    ...
    // kondisi ketika batas member group belum tercapai, ubah di file settings/setting.json
        ...
        await client.simulateTyping(chat.id, true).then(async () => {
            client.sendFileFromUrl(chat.id, 'https://static.wikia.nocookie.net/steins-gate/images/0/07/Amadeuslogo.png')
          client.sendText(chat.id, `Hai minna~, Im AMADEUS BOT. To find out the commands on this bot type #menu`)
        })
        }
    }
    })
